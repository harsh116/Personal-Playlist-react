...
import {openPanel,closePanel,requestLyrics,playAtTop,closeAtTop,
PlayVideo,PauseVideo,toggleRepeat,playNext,playNextAtTop,clearLyrics,
  playNextTrack,playPreviousTrack,toggleQuality} from '../actions';
import 'tachyons';
...
const mapDispatchToProps=(dispatch)=>
{
  ...
  onLyricsRequest: (event)=>
  {
    ...
    return(
    dispatch(requestLyrics(event.target.innerText))
    )
  },
  onPlayAtTop: (event)=>dispatch(playAtTop(event.target.parentElement.parentElement)),
  onCloseVideo: ()=>dispatch(closeAtTop()),
  onPlayVideo: (event)=>dispatch(PlayVideo(event.target.parentElement.parentElement)),
  onPauseVideo: (event)=>dispatch(PauseVideo(event.target.parentElement.parentElement)),
  onRepeat: ()=>dispatch(toggleRepeat()),
  onVideoEnded: (event)=>dispatch(playNext(event.target.parentElement.parentElement)),
  onplayNextAtTop: (event)=>dispatch(playNextAtTop(event.target.parentElement)),
  onResetButton: ()=>dispatch(clearLyrics()),
  onNextTrack: (event)=>dispatch(playNextTrack(event.target.parentElement)),
  onPreviousTrack: (event)=>dispatch(playPreviousTrack(event.target.parentElement)),
...
class App extends Component{
  ...
  render(){
    ...
    const {onLyricsRequest,lyrics,onTopButton,offTopButton,isOnTopButton,onResetButton,title}=this.props;
    ...
    const {repeat,onRepeat,onVideoEnded,nextVideoToBePlayed,
        onplayNextAtTop,nextVideoTitleToBePlayed,onNextTrack,onPreviousTrack,
        onQuality,quality}=this.props;
    let shiftPage='';
    if(paddingLeftVideos===true)
      shiftPage='pl7';

    let fade='';
    if(isOnTopButton===true)
      fade='fade';

  return (
    <div className="">
     <LyricsButton onOpenPanel={onOpenPanel}/>
     <RepeatButton onRepeat={onRepeat}
                    repeat={repeat}
     />
     <QualityButton onQuality={onQuality}
                    quality={quality}
     />
     <Menu repeat={repeat}
          quality={quality}
     ...
     />
      ...
      <div className={shiftPage}>
       <TopVideo videoAtTop={videoAtTop}
       ...
       />
       <VideoArray onLyricsRequest={onLyricsRequest}
